...
BEGIN
  ...
  SELECT r.* INTO v_rest
  FROM public.restaurants AS r
  WHERE lower(trim(r.slug)) = v_slug
  LIMIT 1;
  ...
  FROM public.waiters AS w
  WHERE w.restaurant_id = v_rest.id
    AND lower(trim(w.username)) = v_username
  ...
  INSERT INTO public.waiter_sessions AS ws (waiter_id, token, expires_at)
  VALUES (v_waiter.id, v_token, now() + interval '12 hours');

  RETURN QUERY
  SELECT v_token, v_waiter.id, v_waiter.name, v_rest.id, v_rest.slug, v_rest.name;
END;
$function$;

REVOKE ALL ON FUNCTION public.waiter_login(text, text, text) FROM PUBLIC, anon, authenticated;
